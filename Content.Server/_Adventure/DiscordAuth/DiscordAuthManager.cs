...
using Content.Shared.Adventure.DiscordAuth;
using Content.Shared.Adventure.DiscordMember;
...
public sealed class DiscordAuthManager
{
    ...
    public void Initialize()
    {
        ...
        _cfg.OnValueChanged(ACVars.DiscordAuthEnabled, v => _isEnabled = v, true);
        _cfg.OnValueChanged(ACVars.DiscordAuthApiUrl, v => _apiUrl = v, true);
        _cfg.OnValueChanged(ACVars.DiscordAuthApiKey, v => _apiKey = v, true);
        _cfg.OnValueChanged(ACVars.InfoLinksDiscord, v => _discordLink = v, true);
        _cfg.OnValueChanged(ACVars.DiscordAuthCheckMember, v => _checkMember = v, true);

        _net.RegisterNetMessage<MsgDiscordAuthRequired>();
        _net.RegisterNetMessage<MsgDiscordMemberRequired>();
        _net.RegisterNetMessage<MsgDiscordAuthCheck>(OnAuthCheck);
        _net.RegisterNetMessage<MsgDiscordMemberCheck>(OnMemberCheck);
    ...
    private async void OnAuthCheck(MsgDiscordAuthCheck message)
    {
        ...
        var session = _player.GetSessionByUserId(message.MsgChannel.UserId);
        if (isVerified)
        {
            if (_checkMember)
            {
                ...
                var joinMsg = new MsgDiscordMemberRequired()
                {
                    AuthUrl = joinUrl.Url,
                    QrCode = joinUrl.Qrcode,
